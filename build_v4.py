...
.who-btn.sel-f{border-color:var(--user-f);background:var(--user-f-light);color:var(--user-f-dark)}
.who-btn.sel-m{border-color:var(--user-m);background:var(--user-m-light);color:var(--user-m-dark)}
...
/* v9 pending completion. A green ring marks "will complete", the fill marks WHO. */
.circ.pending{border-color:var(--green);position:relative;font-size:10px;font-weight:700;overflow:visible}
.circ.pending.cc-f{background:var(--user-f-light);color:var(--user-f-dark)}
.circ.pending.cc-m{background:var(--user-m-light);color:var(--user-m-dark)}
.circ.pending.cc-both{background:linear-gradient(135deg,var(--user-f-light) 50%,var(--user-m-light) 50%)}
/* the sweep: a conic wedge grows clockwise over the pending window */
.circ-ring{position:absolute;top:-4px;left:-4px;right:-4px;bottom:-4px;border-radius:50%;
  background:conic-gradient(var(--green) var(--sweep,0turn),transparent 0);
  -webkit-mask:radial-gradient(circle,transparent 62%,#000 64%);
  mask:radial-gradient(circle,transparent 62%,#000 64%);
  animation:sweep var(--pending-ms,2500ms) linear forwards;pointer-events:none}
@keyframes sweep{from{--sweep:0turn}to{--sweep:1turn}}
@property --sweep{syntax:'<angle>';initial-value:0turn;inherits:false}
/* fallback for engines without @property: a simple fade-in ring still reads as a timer */
@supports not (background:conic-gradient(red 0turn,transparent 0)){
  .circ-ring{background:none;border:2px solid var(--green);opacity:.35;animation:ringpulse var(--pending-ms,2500ms) linear forwards}
  @keyframes ringpulse{from{opacity:.15}to{opacity:1}}
}
.circ.committed{background:var(--green);border-color:var(--green)}
.circ.committed::after{content:'\\2713';font-size:11px;color:#fff;font-weight:700}
/* swipe background tinted by who is getting credit */
.swipe-bg.cc-f{background:var(--user-f)}
.swipe-bg.cc-m{background:var(--user-m)}
.swipe-bg.cc-both{background:linear-gradient(135deg,var(--user-f) 50%,var(--user-m) 50%)}
/* Section 9: stale-data nudge */
.boot-error{position:absolute;top:0;left:0;right:0;bottom:0;z-index:60;background:var(--bg);
  display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;padding:32px;text-align:center}
.boot-error.gone{display:none}
.boot-error-title{font-size:17px;font-weight:600;color:var(--text)}
.boot-error-msg{font-size:13.5px;color:var(--text2)}
.boot-error-hint{font-size:12.5px;color:var(--text3);margin-bottom:6px}
.stale-badge{display:none;align-items:center;gap:6px;justify-content:center;padding:6px 10px;
  background:var(--amber-light,#FDF3E0);color:var(--terra);font-size:12px;cursor:pointer;
...
.bc-opt{width:26px;height:26px;border-radius:50%;border:2px solid rgba(255,255,255,.35);
  ...
  cursor:pointer;background:transparent;color:#fff;font-family:inherit;padding:0}
.bc-opt.sel{border-color:#fff}
.bc-opt.bc-f.sel{background:var(--user-f-light);color:var(--user-f-dark)}
.bc-opt.bc-m.sel{background:var(--user-m-light);color:var(--user-m-dark)}
.bc-opt.bc-both.sel{background:linear-gradient(135deg,var(--user-f-light) 50%,var(--user-m-light) 50%);color:transparent}
...
.who-f{background:var(--user-f-light);color:var(--user-f-dark)}
.who-m{background:var(--user-m-light);color:var(--user-m-dark)}
...
.who-both{background:linear-gradient(135deg,var(--user-f-light) 50%,var(--user-m-light) 50%);color:var(--text2)}
...
.who-opt.sel-f{background:var(--user-f-light);color:var(--user-f-dark);border-color:var(--user-f)}
.who-opt.sel-m{background:var(--user-m-light);color:var(--user-m-dark);border-color:var(--user-m)}
...
.ldot{width:8px;height:8px;border-radius:50%;background:var(--user-f);margin-top:5px;flex-shrink:0}
.ldot.b{background:var(--user-m)}.ldot.n{background:var(--border2)}
.ldot.both{background:linear-gradient(135deg,var(--user-f-light) 50%,var(--user-m-light) 50%)}
...
.bar-fill.pur{background:var(--user-f)}.bar-fill.yel{background:var(--user-m)}
...
<div class="app gone" id="main-app">
  ...
  <div class="main">
    ...
    <div class="boot-error gone" id="boot-error">
      <div class="boot-error-title">Couldn't load your data</div>
      <div class="boot-error-msg"></div>
      <div class="boot-error-hint">Your tasks are safe. This is a connection problem.</div>
      <button class="btn primary" onclick="retryBoot()"><i class="ti ti-refresh"></i> Try again</button>
    </div>
    ...
    <div id="v-tasks" class="view gone">
      ...
      <div class="stale-badge" id="stale-badge" onclick="refreshData()"><span class="dot"></span><i class="ti ti-refresh"></i> Tap to refresh</div>
      ...
      <div class="batch-bar" id="batch-bar">
        ...
        <div class="batch-top">
          <div class="batch-credit-lbl">Credit</div>
          <div class="batch-credit" id="batch-credit">
            <button class="bc-opt bc-f" id="bc-f" onclick="pickBatchCredit('Frankie')">F</button>
            <button class="bc-opt bc-m" id="bc-m" onclick="pickBatchCredit('Meredith')">M</button>
            <button class="bc-opt bc-both" id="bc-both" onclick="pickBatchCredit('BOTH')">FM</button>
          </div>
        </div>
...
<div class="modal-bg gone" id="modal-task">
  <div class="modal">
    ...
    <div class="form-row" id="t-owner-row">
      ...
      <div class="form-row" id="t-together-row"><label style="display:flex;align-items:center;gap:9px;cursor:pointer;font-size:13.5px;color:var(--text)">
        <input type="checkbox" id="t-done-together" style="width:17px;height:17px;accent-color:var(--green);cursor:pointer">
        Usually completed by both of us</label>
        <div style="font-size:11.5px;color:var(--text3);margin-top:3px;margin-left:26px">Completion will credit both by default</div></div>
...
// ── v9 PENDING COMPLETION ENGINE ────────────────────────────────────────────
// Tapping the circle no longer writes anything. It cycles a local credit choice and
// starts a 2.5s countdown; ONLY flushPending() ever talks to the server. That single
// write rule is what keeps this from reintroducing the duplicate-completion bugs.
var PENDING_MS=2500;          // countdown before a pending completion commits
var TAP_GUARD_MS=250;         // ignore a second tap on the same circle inside this
var RECENT_COMMIT_MS=10000;   // ignore a server payload that resurrects a just-committed task
var UNDO_MS=5000;             // how long the undo toast lives
// task_id -> {credit, idx, timer, startedAt, lastTapAt, task}
// credit is a completed_by string, or null meaning "cleared, do not commit"
var _pending={};
// task_id -> timestamp of a successful commit, so a stale refresh cannot resurrect it
var _recentCommit={};

function pendingCount(){return Object.keys(_pending).length;}
// Section 6: automatic syncing is only safe when nothing is mid-commit. Kept as its own
// predicate so the rule is assertable rather than buried in a timer callback.
function bgSyncAllowed(){return pendingCount()===0;}
function isPending(tid){return Object.prototype.hasOwnProperty.call(_pending,tid);}
function otherPerson(name){var o=_PEOPLE.filter(function(n){return n!==name;});return o[0]||name;}
function bothCredit(){return _PEOPLE.join(',');}
function isDoneTogether(t){var v=t&&t.done_together;return v===true||v==='true'||v==='TRUE'||v===1||v==='1';}

// Section 14: the default credit derives from the ASSIGNMENT, not from who tapped.
function defaultCreditFor(t){
  if(String(t.scope||'household')==='personal')return String(t.owner||currentUser);
  var owner=String(t.owner||'');
  if(owner.indexOf(',')>=0)return bothCredit();       // assigned to both
  if(isDoneTogether(t))return bothCredit();           // flagged as usually done together
  if(owner)return owner;                              // assigned to one person
  return currentUser;                                 // either of us
}
// The full cycle. Last entry is always null, meaning clear/cancel.
function creditCycleFor(t){
  if(String(t.scope||'household')==='personal')return [String(t.owner||currentUser),null];
  var owner=String(t.owner||''),both=bothCredit();
  if(owner.indexOf(',')>=0||isDoneTogether(t))return [both,currentUser,otherPerson(currentUser),null];
  if(owner)return [owner,otherPerson(owner),both,null];
  return [currentUser,otherPerson(currentUser),both,null];
}
function creditClass(credit){
  if(!credit)return '';
  var f=hasPerson(credit,'Frankie'),m=hasPerson(credit,'Meredith');
  return f&&m?'cc-both':f?'cc-f':m?'cc-m':'';
}
function creditLetter(credit){
  if(!credit)return '';
  var f=hasPerson(credit,'Frankie'),m=hasPerson(credit,'Meredith');
  if(f&&m)return '';
  return f?'F':m?'M':'';
}
function buzz(credit,cleared){
  if(cleared||!navigator.vibrate)return;
  try{navigator.vibrate(hasPerson(credit,'Frankie')&&hasPerson(credit,'Meredith')?[15,50,15]:[15]);}catch(e){}
}

// Paint one circle to match its pending state. Kept separate from renderTasks so a
// cycle tap can update in place without rebuilding the list (Section 12D relies on
// renderTasks reading the same _pending map when it does rebuild).
function paintCircle(circ,tid){
  if(!circ)return;
  var p=_pending[tid];
  circ.className='circ'+(selectMode&&selectedTaskIds.has(tid)?' sel':'');
  circ.innerHTML='';
  if(!p||!p.credit)return;
  circ.className+=' pending '+creditClass(p.credit);
  var letter=creditLetter(p.credit);
  if(letter)circ.appendChild(document.createTextNode(letter));
  var ring=document.createElement('span');ring.className='circ-ring';
  // restart the sweep from wherever this pending window currently is
  ring.style.setProperty('--pending-ms',PENDING_MS+'ms');ring.style.animationDuration=PENDING_MS+'ms';
  circ.appendChild(ring);
}
function repaintPending(tid){
  var wrap=document.querySelector('.tc-wrap[data-task-id="'+tid+'"]');
  if(wrap)paintCircle(wrap.querySelector('.circ'),tid);
}

// THE tap handler. Never writes. Never mutates state.tasks.
function cycleCompletion(t){
  var tid=t.task_id,now=Date.now();
  if(recentlyCommitted(tid))return;                 // committed moments ago, do not re-arm
  var p=_pending[tid];
  if(p&&(now-p.lastTapAt)<TAP_GUARD_MS)return;        // accidental double tap
  // Section 2: starting a new pending window commits any other pending card first.
  var others=Object.keys(_pending).filter(function(k){return k!==tid;});
  if(!p&&others.length)flushPending(others);
  var cycle=creditCycleFor(t);
  var idx=p?(p.idx+1)%cycle.length:0;
  var credit=cycle[idx];
  if(p&&p.timer)clearTimeout(p.timer);
  _pending[tid]={credit:credit,idx:idx,startedAt:now,lastTapAt:now,task:t,timer:null};
  buzz(credit,!credit);
  _pending[tid].timer=setTimeout(function(){flushPending([tid]);},PENDING_MS);
  repaintPending(tid);
  setSyncState('loading',credit?'Completing...':'Cancelled');
}
// Section 4: any non-cycle interaction with a pending card cancels it.
function cancelPending(tid,repaint){
  var p=_pending[tid];
  if(!p)return false;
  if(p.timer)clearTimeout(p.timer);
  delete _pending[tid];
  if(repaint!==false)repaintPending(tid);
...
function recentlyCommitted(tid){
  var t=_recentCommit[tid];
  if(!t)return false;
  if(Date.now()-t>RECENT_COMMIT_MS){delete _recentCommit[tid];return false;}
  return true;
}
function completionHidden(tid){return _recentlyCompleted.has(tid)||recentlyCommitted(tid);}

// THE ONLY PATH THAT WRITES A COMPLETION. Section 12A/B/F.
function flushPending(ids){
  var list=(ids||Object.keys(_pending)).slice();
  var items=[];
  // Remove EVERY id from the map synchronously, before any async work, so a concurrent
  // flush trigger cannot find and re-commit the same task.
  list.forEach(function(tid){
    var p=_pending[tid];
    if(!p)return;
    if(p.timer)clearTimeout(p.timer);
    delete _pending[tid];
    if(!p.credit){repaintPending(tid);return;}        // cleared: no API call at all
    items.push({task_id:tid,credit:p.credit,task:p.task});
  });
  if(!items.length){if(!pendingCount())setSyncState('ok','Ready');return Promise.resolve();}

  var picks=items.map(function(it){var t=it.task;return {
    task_id:t.task_id,task_name:t.name,type:t.type,recurrence_days:t.recurrence_days,
    weekday:t.weekday,day_of_month:t.day_of_month,sched_freq:t.sched_freq,
    sched_interval:t.sched_interval,sched_month:t.sched_month,due_date:t.due_date,
    end_date:t.end_date,scope:t.scope||'household',completed_by:it.credit};});
  var tids=items.map(function(it){return it.task_id;});
  tids.forEach(function(tid){_recentlyCompleted.add(tid);_recentCommit[tid]=Date.now();});
  collapseCards(tids);
  setSyncState('loading','Logging...');
  // One request for the whole flush, never one per task.
  return apiPost({action:'batchComplete',data:{tasks:picks,completed_by:currentUser}})
    .then(function(res){
      showUndoToast(items,res&&res.log_ids?res.log_ids:null);
      scheduleBgSync(SYNC_FAST);
    })
    .catch(function(){
      // Section 12E: no automatic retry. Put the cards back and say so.
      tids.forEach(function(tid){_recentlyCompleted.delete(tid);delete _recentCommit[tid];});
      renderTasks();
      showToast("Couldn't confirm "+(items.length>1?items.length+' completions':'the completion'));
      setSyncState('err','Could not save');
      scheduleBgSync();
    });
}
function collapseCards(tids){
  tids.forEach(function(tid){
    var wrap=document.querySelector('.tc-wrap[data-task-id="'+tid+'"]');
    if(!wrap)return;
    wrap.style.height=wrap.offsetHeight+'px';wrap.style.transition='all .2s';
    var circ=wrap.querySelector('.circ');
    if(circ){circ.className='circ committed';circ.innerHTML='';}
    setTimeout(function(){wrap.style.height='0';wrap.style.opacity='0';wrap.style.overflow='hidden';},180);
  });
}
...
// ── BOOT ──────────────────────────────────────────────────
// Boot as soon as the DOM is parsed, NOT on window.onload. onload waits for every
// subresource, including the Google Fonts stylesheet, so a slow font server delayed the
// cached paint by the full font timeout: measured at 12.9s in the browser harness. The
// whole point of the cache is that nothing on the network gates the first screen.
function bootApp(){
  var s=localStorage.getItem('loonhq_user');
  if(s==='Frankie'||s==='Meredith'){currentUser=s;showApp();bootData();apiGet({action:'ping'}).catch(function(){});}
  else showLogin();}
...
function selectUser(name,btn){
  ...
  showApp();
  bootData();
  apiGet({action:'ping'}).catch(function(){});   // fire-and-forget: never awaited, so a cold
                                                 // container cannot delay the data fetch
}
...
var API_TIMEOUT=25000;      // writes: short, because a retry could duplicate
var GET_TIMEOUT=60000;      // reads: long, because a cold Apps Script container is slow
                            // and a read can never duplicate anything
function _fetchTimeout(url,opts,ms){
  ...
  timer=setTimeout(function(){timer=null;ctl.abort();},ms||API_TIMEOUT);
...
function apiGet(params){var url=API+'?'+Object.entries(params).map(function(p){return p[0]+'='+encodeURIComponent(p[1]);}).join('&');return _fetchTimeout(url,null,GET_TIMEOUT).then(_apiParse);}
...
function refreshData(silent){
  // A manual refresh is also a flush trigger. Commit what is pending, then fetch.
  if(!silent&&pendingCount()){
    return flushPending().then(function(){return refreshData(silent);});
  }
  ...
  return apiGet({action:'getAllData'}).then(function(data){
    ...
    _lastFetch=Date.now();
    // If nothing actually changed there is no reason to rebuild the list underneath the
    // user. Cheap to compute next to a full render.
    var sig=JSON.stringify(data);
    var unchanged=(sig===_lastPayloadSig);
    _lastPayloadSig=sig;
    ...
    document.getElementById('loader').style.display='none';
    document.getElementById('v-'+currentView).classList.remove('gone');
    saveStateCache();                       // so the next launch paints instantly
    if(!unchanged)renderAll();
    setSyncState('ok','Synced '+new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}));
  }).catch(function(err){
    document.getElementById('loader').style.display='none';
    document.getElementById('v-'+currentView).classList.remove('gone');
    // Server reachable but rejected the request: keep cached data, don't fall back to STATIC_ASSETS.
    if(err&&err.apiError){setSyncState('err','Sync failed');showBootError('The server rejected the request.');return;}
    state.assets=STATIC_ASSETS;state.maintenance_logs=[];renderAll();setSyncState('err','Offline');
    showBootError(err&&err.timedOut?'The server took too long to answer.':'Could not reach the server.');
  });
}
...
function scheduleBgSync(delay){
  if(_bgSyncTimer)clearTimeout(_bgSyncTimer);
  _bgSyncTimer=setTimeout(function(){
    _bgSyncTimer=null;
    // Section 6: a payload landing mid-cycle would rebuild the list underneath the user's
    // finger. Hold off and re-arm; the flush itself schedules the real reconcile.
    if(!bgSyncAllowed()){scheduleBgSync(SYNC_SLOW);return;}
    refreshData(true);
  },delay||SYNC_SLOW);
}
function rebuildTaskIndex(){_taskById={};(state.tasks||[]).forEach(function(t){_taskById[t.task_id]=t;});}
// Every lookup map that is derived from state. Called after a server payload AND after the
// cache is restored, so both paths leave the app in exactly the same shape.
...
// The app used to show nothing until Apps Script answered, which on a cold container is
// 10-20 seconds of blank screen. The last known state is now kept in localStorage and
// rendered immediately, with the server fetch happening behind it.
var CACHE_KEY='loonhq_state_v1';
var CACHE_STALE_MS=86400000;     // 24h: still render from cache, but say we are syncing
var _lastPayloadSig=null;        // so an unchanged payload does not trigger a re-render
function saveStateCache(){
  try{
    localStorage.setItem(CACHE_KEY,JSON.stringify({
      at:Date.now(),
      tasks:state.tasks,projects:state.projects,subtasks:state.subtasks,
      grocery:state.grocery,lists:state.lists,task_log:state.task_log,
      assets:state.assets,maintenance_logs:state.maintenance_logs
    }));
  }catch(e){ /* quota, private mode, whatever: the cache is an optimisation, never required */ }
}
function loadStateCache(){
  try{
    ...
    if(!c||!c.at||!Array.isArray(c.tasks))return null;   // ignore anything malformed
    return c;
  }catch(e){return null;}
}
function clearStateCache(){try{localStorage.removeItem(CACHE_KEY);}catch(e){}}
function applyCachedState(c){
  state.tasks=c.tasks||[];state.projects=c.projects||[];state.subtasks=c.subtasks||[];
  state.grocery=c.grocery||[];state.lists=c.lists||[];state.task_log=c.task_log||[];
  state.assets=(c.assets&&c.assets.length)?c.assets:STATIC_ASSETS;
  state.maintenance_logs=c.maintenance_logs||[];
  rebuildDerivedIndexes();
}
// Startup: paint from cache first, then reconcile in the background. Only the very first
// launch (or cleared storage) ever sees a loading screen.
// Shown only when the app has NOTHING to display: no cache and the first fetch failed.
// Before this, that situation produced a silent blank screen with no way forward, and
// because nothing was ever cached the next launch failed exactly the same way.
function showBootError(reason){
  if((state.tasks||[]).length)return;          // we have something on screen; leave it alone
  var el=document.getElementById('boot-error');
  if(!el)return;
  var msg=el.querySelector('.boot-error-msg');
  if(msg)msg.textContent=reason||'Could not load your data.';
  el.classList.remove('gone');
}
function hideBootError(){var el=document.getElementById('boot-error');if(el)el.classList.add('gone');}
function retryBoot(){
  hideBootError();
  var l=document.getElementById('loader');if(l)l.style.display='flex';
  fetchWithRetry(2);
}
// A read is safe to retry, so give a cold container a couple of chances before giving up.
function fetchWithRetry(triesLeft){
  return refreshData(false).then(function(){
    if((state.tasks||[]).length||!triesLeft)return;
    if(document.getElementById('boot-error')&&!document.getElementById('boot-error').classList.contains('gone')){
      hideBootError();
      var l=document.getElementById('loader');if(l)l.style.display='flex';
      return fetchWithRetry(triesLeft-1);
    }
  });
}
function bootData(){
  var c=loadStateCache();
  if(!c){
    fetchWithRetry(2);           // nothing cached: show the loader and retry a cold server
    return false;
  }
  applyCachedState(c);
  var loader=document.getElementById('loader');if(loader)loader.style.display='none';
  var view=document.getElementById('v-'+currentView);if(view)view.classList.remove('gone');
  renderAll();
  var age=Date.now()-c.at;
  if(age>CACHE_STALE_MS)setSyncState('loading','Syncing...');
  else setSyncState('ok','Showing saved data');
  refreshData(true);             // silent: the screen is already populated
  return true;
}
...
// ── v9 UNDO TOAST (Section 7) ───────────────────────────────────────────────
// Shown after a commit. Tapping Undo removes the log row and restores the task.
// Only one toast at a time; a new one replaces the old.
var _undoItems=null,_undoLogIds=null,_undoTimer=null;
function showUndoToast(items,logIds){
  var t=document.getElementById('toast-msg');if(!t)return;
  if(_toastTimer){clearTimeout(_toastTimer);_toastTimer=null;}
  if(_undoTimer){clearTimeout(_undoTimer);_undoTimer=null;}
  _undoItems=items;_undoLogIds=logIds||null;_toastRetry=null;
  t.textContent=items.length>1
    ? items.length+' tasks completed  \u00b7  Undo all'
    : (items[0].task.name||'Task')+' completed  \u00b7  Undo';
  t.classList.add('tappable');t.classList.add('on');
  _undoTimer=setTimeout(function(){_undoTimer=null;_undoItems=null;_undoLogIds=null;hideToast();},UNDO_MS);
}
function undoLastCompletion(){
  var items=_undoItems,logIds=_undoLogIds;
  _undoItems=null;_undoLogIds=null;
  if(_undoTimer){clearTimeout(_undoTimer);_undoTimer=null;}
  hideToast();
...
function showToast(msg,retry){
  ...
  // Only one toast is ever visible, so a new one disarms any pending undo. Without this a
  // later failure toast would still fire undoLastCompletion when tapped.
  if(_undoTimer){clearTimeout(_undoTimer);_undoTimer=null;}
  _undoItems=null;_undoLogIds=null;
...
function onToastTap(){
  if(_undoItems){undoLastCompletion();return;}   // v9 undo takes priority
  var r=_toastRetry;hideToast();if(_toastTimer){clearTimeout(_toastTimer);_toastTimer=null;}if(r)r();
}
...
document.addEventListener('visibilitychange',function(){
  // Backgrounding, phone lock or a browser tab switch must not strand a pending
  // completion, so commit before we lose the foreground.
  if(document.hidden){flushPending();return;}
  // Advisory only. Returning to the app no longer triggers a fetch on its own; the badge
  // invites one. Fetching happens when the user taps the badge or the refresh button.
  if(_lastFetch&&(Date.now()-_lastFetch)>STALE_MS)showStaleBadge();
});
// Section 9: a subtle nudge when the data on screen may be older than the other person's
// changes. Purely advisory; tapping it flushes and refetches.
var STALE_MS=30000;
var STALE_CHECK_MS=120000;   // how often we LOOK; the check never fetches by itself
function showStaleBadge(){var b=document.getElementById('stale-badge');if(b)b.classList.add('on');}
function hideStaleBadge(){var b=document.getElementById('stale-badge');if(b)b.classList.remove('on');}
function checkStale(){
...
function startStaleWatch(){
  ...
  _staleTimer=setInterval(checkStale,STALE_CHECK_MS);
}
...
function renderTasks(){
  ...
  var all=state.tasks.filter(function(t){
    if(completionHidden(t.task_id))return false;
...
function makeTaskCard(t){
  ...
  // v9.3: a newly added task renders as a normal, interactive card. It used to get a
  // dimmed ".loading" treatment with a "saving..." suffix that sat there for the whole
  ...
  circ.addEventListener('click',function(e){
    ...
    cycleCompletion(t);   // v9: cycles credit locally, never writes
  });
  // Section 12D: a rebuilt list must show a pending card in its pending visual, not as
  // a fresh active card, so there is never a second completion target for one task.
  if(isPending(t.task_id))paintCircle(circ,t.task_id);
  ...
  mb.addEventListener('click',function(e){e.stopPropagation();cancelPending(t.task_id);openTaskMenu(e,t);});
  div.appendChild(mb);
...
function attachSwipe(cardEl,wrapEl,swipeBg,task){
  ...
  cardEl.addEventListener('touchend',function(){
    ...
    if(curX>THRESH){
      // Section 3: the fast path. Commit straight away at the assignment-derived default,
      // flashing that person's colour so the user sees who got credit.
      cancelPending(task.task_id,false);
      var credit=defaultCreditFor(task);
      var cc=creditClass(credit);
      if(cc){swipeBg.className='swipe-bg swipe-complete '+cc;}
      var cEl=cardEl.querySelector('.circ');
      if(cEl){cEl.className='circ pending '+cc;cEl.innerHTML='';var L=creditLetter(credit);if(L)cEl.textContent=L;}
      cardEl.style.transform='translateX(110%)';
      _pending[task.task_id]={credit:credit,idx:0,startedAt:Date.now(),lastTapAt:Date.now(),task:task,timer:null};
      setTimeout(function(){flushPending([task.task_id]);},160);
    }
    else if(curX<-THRESH){cardEl.style.transform='';swipeBg.style.display='none';cancelPending(task.task_id);openSnooze(task);}
...
function enterBatch(){
  ...
  _batchCredit=null;syncBatchCredit();
  applyBatchInset();
  updateBatchCount();
}
...
// Section 8: which credit the whole batch gets. Defaults to the logged-in user.
var _batchCredit=null;
function batchCreditValue(){return _batchCredit==='BOTH'?bothCredit():(_batchCredit||currentUser);}
function pickBatchCredit(v){_batchCredit=v;syncBatchCredit();}
function syncBatchCredit(){
  var cur=_batchCredit||currentUser;
  [['bc-f','Frankie'],['bc-m','Meredith'],['bc-both','BOTH']].forEach(function(pair){
    var el=document.getElementById(pair[0]);
    if(el)el.classList.toggle('sel',cur===pair[1]);
  });
}
function batchCompleteSelected(){
  var ids=[];selectedTaskIds.forEach(function(id){ids.push(id);});
  if(!ids.length)return;
  var credit=batchCreditValue();          // Section 8: one credit for the whole selection
  exitBatch();
  var armed=[];
  ids.forEach(function(id){
    var t=(state.tasks||[]).find(function(x){return x.task_id===id;});
    if(!t)return;
    _pending[id]={credit:credit,idx:0,startedAt:Date.now(),lastTapAt:Date.now(),task:t,timer:null};
    armed.push(id);
  });
  // Routed through the same flush as everything else, so flushPending stays the single
  // code path that writes a completion (Section 12A).
  return flushPending(armed);
}
...
// The immediate-commit path: swipe-right and the card action menu. It arms the pending map
// at the assignment-derived default and flushes at once, so flushPending remains the ONLY
// code that ever writes a completion (Section 12A). No cycling, no countdown.
function handleComplete(t){
  var tid=t.task_id;
  if(recentlyCommitted(tid))return;                 // just committed; ignore a repeat tap
  if(isPending(tid)&&_pending[tid].credit)return;   // already armed, let it commit once
  _pending[tid]={credit:defaultCreditFor(t),idx:0,startedAt:Date.now(),lastTapAt:Date.now(),task:t,timer:null};
  return flushPending([tid]);
}
...
function openAddTask(){
  ...
  var dtA=document.getElementById('t-done-together');if(dtA)dtA.checked=false;
...
function openEditTask(t){
  cancelPending(t.task_id);   // Section 4: latest action wins
  populateAssetDropdown();populateProjectDropdown();
  editingTask=t;pickedScope=t.scope||'household';pickedOwner=t.owner||'';pickedUrgency=t.urgency_window||'this_week';
  var dtE=document.getElementById('t-done-together');if(dtE)dtE.checked=isDoneTogether(t);
...
function submitTask(){
  ...
  var type=document.getElementById('t-type').value;
  var data={name:name,type:type,scope:pickedScope,owner:pickedScope==='personal'?currentUser:pickedOwner,done_together:(document.getElementById('t-done-together')||{}).checked?'true':'',notes:document.getElementById('t-notes').value.trim(),status:'active'};
...
function setStatsScope(s){
  flushPending();   // Section 5: scope switch commits
  statsScope=s;
...
function renderTrendChart(){
  ...
  wrapEl.innerHTML='<div class="trend-title">Completions over time</div><svg width="100%" viewBox="0 0 '+W+' '+H+'" style="overflow:visible">'+yLines+'<polyline points="'+fPts+'" fill="none" stroke="var(--user-f)" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/><polyline points="'+mPts+'" fill="none" stroke="var(--user-m)" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>'+xLabels+'</svg><div class="trend-legend"><span><span class="trend-legend-dot" style="background:var(--purple)"></span>Frankie</span><span><span class="trend-legend-dot" style="background:var(--yellow)"></span>Meredith</span></div>';
}
