...
var _bgSyncTimer=null,_lastFetch=0;
...
function apiGet(params){var url=API+'?'+Object.entries(params).map(function(p){return p[0]+'='+encodeURIComponent(p[1]);}).join('&');return fetch(url).then(_apiParse);}
function apiPost(body){return fetch(API,{method:'POST',body:JSON.stringify(body)}).then(_apiParse);}
...
function refreshData(silent){
  ...
  return apiGet({action:'getAllData'}).then(function(data){
    ...
    _taskById={};(state.tasks||[]).forEach(function(t){_taskById[t.task_id]=t;});
...
function scheduleBgSync(){if(_bgSyncTimer)clearTimeout(_bgSyncTimer);_bgSyncTimer=setTimeout(function(){_bgSyncTimer=null;refreshData(true);},3000);}
...
if(typeof window!=='undefined'&&window.addEventListener)window.addEventListener('unhandledrejection',function(e){if(e.reason&&e.reason.apiError){e.preventDefault();showToast("Couldn't save — try again");setSyncState('err','Save failed');scheduleBgSync();}});
...
function batchCompleteSelected(){
  ...
  apiPost({action:'batchComplete',data:{tasks:picks,completed_by:currentUser}}).then(function(){scheduleBgSync();}).catch(function(){picks.forEach(function(p){_recentlyCompleted.delete(p.task_id);});renderTasks();showToast("Couldn't complete tasks");setSyncState('err','Could not complete');});
}
...
function confirmBatchSnooze(){
  ...
  Promise.all(calls).then(function(){scheduleBgSync();}).catch(function(){updates.forEach(function(u){u.task.due_date=prevDates[u.task.task_id];});renderTasks();showToast("Couldn't snooze tasks");setSyncState('err','Could not snooze');});
}
...
function confirmSnooze(){
  ...
  apiPost({action:'snoozeTask',data:{task_id:tid,until_date:targetDate,snoozed_by:currentUser}}).then(function(){scheduleBgSync();}).catch(function(){var t2=state.tasks.find(function(x){return x.task_id===tid;});if(t2)t2.due_date=prevDue;renderTasks();showToast("Couldn't snooze — try again");setSyncState('err','Could not snooze');});
}
...
function submitTask(){
  ...
  if(editingTask){
    ...
    if(currentView==='tasks')renderTasks();
    apiPost({action:'updateTask',data:{task_id:etid,updates:data,updated_by:currentUser}}).then(function(){scheduleBgSync();}).catch(function(){var t2=state.tasks.find(function(x){return x.task_id===etid;});if(t2)Object.assign(t2,prevTask);renderTasks();showToast("Couldn't save changes");setSyncState('err','Could not save');});
  }else{
    ...
    if(currentView==='tasks')renderTasks();
    apiPost({action:'addTask',data:data}).then(function(){state.tasks=state.tasks.filter(function(x){return x.task_id!==tempId;});delete _taskById[tempId];scheduleBgSync();}).catch(function(){state.tasks=state.tasks.filter(function(x){return x.task_id!==tempId;});delete _taskById[tempId];renderTasks();showToast("Couldn't save task");setSyncState('err','Could not save');});
  }
}
function deleteEditingTask(){
  ...
  apiPost({action:'deleteTask',data:{task_id:dtid}}).then(function(){scheduleBgSync();}).catch(function(){state.tasks=prevTasks;_taskById={};state.tasks.forEach(function(x){_taskById[x.task_id]=x;});renderTasks();showToast("Couldn't delete task");setSyncState('err','Could not delete');});
}
...
function makeGrocEl(item){
  ...
  d.addEventListener('drop',function(e){
    ...
    }).catch(function(){renderGrocery();showToast("Couldn't save order");});
  });
  return d;
}
function editGrocItem(id,textEl,item){
  ...
  function save(){
    ...
    apiPost({action:'updateGrocery',data:{item_id:id,updates:{name:val}}}).catch(function(){textEl.textContent=current;if(it)it.name=current;showToast("Couldn't rename item");});
  }
...
function markIncomplete(){
  ...
  apiPost({action:'uncompleteTask',data:{log_id:_histMenuLog.log_id,task_id:_histMenuLog.task_id}}).then(function(){scheduleBgSync();});
}
function deleteTaskLogEntry(){
  ...
  apiPost({action:'deleteTaskLog',data:{log_id:_histMenuLog.log_id}}).then(function(){scheduleBgSync();}).catch(function(){setSyncState('err','Could not delete');showToast("Couldn't delete entry");});
}
...
function submitReassign(){
  ...
  apiPost({action:'reassignCompletion',data:{log_id:_histMenuLog.log_id,task_id:_histMenuLog.task_id,completed_by:_reassignPerson,scope:_reassignScope}}).then(function(){scheduleBgSync();});
}
// ── MODALS ────────────────────────────────────────────────
...
function submitProject(){
  ...
  if(editingProject){apiPost({action:'updateProject',data:{project_id:editingProject.project_id,updates:data}}).then(function(){scheduleBgSync();});}
  else{apiPost({action:'addProject',data:data}).then(function(){scheduleBgSync();});}
}
function deleteEditingProject(){
  if(!editingProject)return;
  if(!confirm('Delete this project and all its tasks?'))return;
  closeModal('modal-project');setSyncState('loading','Deleting...');
  apiPost({action:'deleteProject',data:{project_id:editingProject.project_id}}).then(function(){scheduleBgSync();});
}
...
function submitGrocery(){var n=document.getElementById('g-name').value.trim();if(!n)return;closeModal('modal-grocery');setSyncState('loading','Saving...');var tempId='tmp_'+Date.now();var cat=document.getElementById('g-cat').value;var tempItem={item_id:tempId,name:n,category:cat,added_by:currentUser,status:'need',_temp:true};state.grocery.push(tempItem);renderGrocery();apiPost({action:'addGroceryItem',data:{name:n,category:cat,added_by:currentUser}}).then(function(){state.grocery=state.grocery.filter(function(g){return g.item_id!==tempId;});scheduleBgSync();}).catch(function(){state.grocery=state.grocery.filter(function(g){return g.item_id!==tempId;});renderGrocery();showToast("Couldn't add item");setSyncState('err','Could not save');});}
