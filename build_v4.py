...
.tag-sched-m{background:var(--purple-light);color:var(--purple)}
...
/* ACTIVITY */
.stats-range-bar{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:4px}
.range-btn{font-size:12px;padding:5px 12px;border-radius:20px;border:1px solid var(--border);background:var(--bg);color:var(--text2);cursor:pointer;font-family:inherit;transition:all .12s;white-space:nowrap}
    ...
    <div id="v-metrics" class="view gone">
      ...
      <div id="metrics-stats-panel" class="scroll">
        <div class="stats-range-bar">
          <button class="range-btn" data-days="15" onclick="setStatsRange(15)">Last 15 days</button>
          <button class="range-btn on" data-days="30" onclick="setStatsRange(30)">Last 30 days</button>
          <button class="range-btn" data-days="60" onclick="setStatsRange(60)">Last 60 days</button>
          <button class="range-btn" data-days="90" onclick="setStatsRange(90)">Last 90 days</button>
        </div>
...
function handleComplete(t){
  ...
  apiPost({action:'completeTask',data:{task_id:t.task_id,task_name:t.name,type:t.type,recurrence_days:t.recurrence_days,weekday:t.weekday,day_of_month:t.day_of_month,sched_month:t.sched_month,sched_freq:t.sched_freq,sched_interval:t.sched_interval,due_date:t.due_date,end_date:t.end_date,completed_by:currentUser,scope:t.scope||'household',notes:''}}).then(function(){refreshData(true).then(function(){if(ttype==='scheduled'||ttype==='interval'){_recentlyCompleted.delete(tid);renderTasks();}});}).catch(function(){setSyncState('error','Could not complete');refreshData(true);});
}
...
function submitTask(){
  ...
  if(editingTask){apiPost({action:'updateTask',data:{task_id:editingTask.task_id,updates:data,updated_by:currentUser}}).then(function(){refreshData(true);});}
...
function deleteTaskLogEntry(){
  ...
  apiPost({action:'deleteTaskLog',data:{log_id:_histMenuLog.log_id}}).then(function(){refreshData(true);}).catch(function(){setSyncState('error','Could not delete');refreshData(true);});
}
